...
# EXTERNAL-IP: 10.3.0.22
# 2. Kubernetes service ашиглан domain үүсгэх (ExternalName эсвэл headless service)
apiVersion: v1
kind: Service
metadata:
  name: mariadb
...
spec:
  type: ExternalName
  externalName: mariadb.openstack.svc.ctr.must


# on az n

# check connection to mariadb
kubectl get svc -n openstack | grep mariadb
...
helm upgrade --install mariadb openstack-helm/mariadb \
  --namespace openstack \
  --values ~/overrides/mariadb/mariadb.yaml 
...
cat > ~/overrides/mariadb/mariadb.yaml << 'EOF'
---
labels:
  server:
