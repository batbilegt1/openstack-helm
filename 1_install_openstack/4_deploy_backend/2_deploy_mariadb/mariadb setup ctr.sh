# kubectl label node  az-w1 az-w2 az-w3 openstack-control-plane=enabled
...
tee > ~/osh/overrides/mariadb/mariadb.yaml <<EOF
---
labels:
  server:
...
helm upgrade --install mariadb openstack-helm/marikeystone-api-6cc786fc9-cfrpradb \
    --namespace=openstack \
